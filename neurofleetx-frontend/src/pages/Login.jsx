...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // 🌓 Try Demo Bypass first (for testing)
      const isDemo = ["admin", "manager", "driver", "customer"].some(role => 
        form.email.toLowerCase().includes(role)
      );

      let res;
      if (isDemo) {
        console.log("⚡ Proceeding with Demo Bypass...");
        ...
        // Sync with backend (or use mock if backend is down)
        try {
          res = await authApi.login(form.email, form.password);
        } catch (apiErr) {
          console.warn("⚠️ Backend sync failed, using Firebase data directly");
          // Create a mock response for frontend to continue
          res = {
            data: {
              token: await firebaseUser.getIdToken(),
              roles: userSnap.exists() ? userSnap.data().roles : ["CUSTOMER"],
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="relative z-20 backdrop-blur-3xl rounded-[2.5rem] shadow-2xl p-12 w-[480px] border border-white/5"
        style={{
          background:
            "linear-gradient(145deg, rgba(15, 23, 42, 0.85) 0%, rgba(2, 6, 23, 0.95) 100%)",
          boxShadow: `
      ...
      >
          ...
          >
            ...
            >
              <motion.input
                ...
                className="w-full p-4 pl-12 pr-12 bg-slate-900/40 backdrop-blur-md text-white border border-white/10 rounded-2xl focus:outline-none placeholder-white/40 focus:border-blue-500/50 transition-all duration-300"
                value={form.email}
          ...
          >
            ...
            >
              <motion.input
                ...
                className="w-full p-4 pl-12 pr-12 bg-slate-900/40 backdrop-blur-md text-white border border-white/10 rounded-2xl focus:outline-none placeholder-white/40 focus:border-purple-500/50 transition-all duration-300"
                value={form.password}
                onChange={handleChange}
        ...
        {/* Demo Credentials Quick Access - Moved higher for visibility */}
        <motion.div
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <span className="text-[10px] font-bold text-blue-400/60 uppercase tracking-[0.2em]">
              Demo Quick Access ⚡
            </span>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { role: "Admin", email: "admin@neurofleetx.com", icon: "🛡️", color: "from-blue-500/10" },
              { role: "Manager", email: "manager@neurofleetx.com", icon: "📊", color: "from-purple-500/10" },
              { role: "Driver", email: "driver@neurofleetx.com", icon: "🚛", color: "from-orange-500/10" },
              { role: "Customer", email: "customer@neurofleetx.com", icon: "👤", color: "from-emerald-500/10" }
            ].map((demo, idx) => (
              <motion.button
                key={demo.role}
                type="button"
                ...
                whileTap={{ scale: 0.95 }}
                onClick={() => {
                  setForm({ email: demo.email, password: "demo" });
                }}
                className={`flex items-center gap-3 p-2.5 rounded-xl border border-white/5 bg-gradient-to-br ${demo.color} to-transparent backdrop-blur-md transition-all group`}
              >
        ...
        {/* Corner accents - Enhanced glow */}
        <div className="absolute top-6 left-6 w-16 h-16 border-t-2 border-l-2 border-blue-400/30 rounded-tl-3xl pointer-events-none shadow-[0_0_15px_rgba(59,130,246,0.2)]" />
        <div className="absolute bottom-6 right-6 w-16 h-16 border-b-2 border-r-2 border-purple-400/30 rounded-br-3xl pointer-events-none shadow-[0_0_15px_rgba(167,139,250,0.2)]" />
      </motion.div>
