...
import { fetchVehicles, createVehicle, updateVehicle, deleteVehicle } from "../api/vehicleApi";
...
export default function VehicleDashboard() {
  ...
  const handleAddEdit = async (vehicle) => {
    try {
      if (vehicle.id) {
        const res = await updateVehicle(vehicle.id, vehicle);
        dispatch({
        ...
        dispatch({ type: VehicleActionTypes.ADD_VEHICLE, payload: res });
      }
    } catch (error) {
