...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ Sync with backend (optional/best-effort)
      try {
        const res = await registerUser({ fullName, email, password, role });
        setMessage(res.data.message || "Registration success!");
      } catch (backendErr) {
        console.warn("⚠️ Backend sync failed, but Firebase account created:", backendErr);
        setMessage("Account created successfully (Cloud Secure)!");
      }
